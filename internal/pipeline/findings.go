...
func mergeFindingsJSON(existingRaw, additionalRaw string) string {
	...
	merged := types.Findings{Summary: existing.Summary, Tested: existing.Tested, TestingSummary: existing.TestingSummary, RiskLevel: existing.RiskLevel, RiskRationale: existing.RiskRationale, RiskScope: existing.RiskScope, DocumentationRequired: existing.DocumentationRequired, DocumentationRationale: existing.DocumentationRationale}
